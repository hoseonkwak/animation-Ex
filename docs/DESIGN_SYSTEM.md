...
1. `idle`: 선택용 썸네일과 실행 버튼
2. `loading`: 중립 skeleton과 상태 문구
3. `running`: 실제 CodePen iframe
4. `paused`: 화면 밖에서 iframe을 제거하고 썸네일로 돌아온 상태
5. `error`: 다시 실행과 상세 열기

썸네일은 예제 선택용 정적 이미지일 뿐이다. 클릭한 카드에서는 실제 실행 iframe을 표시한다.
...
- 원본 CodePen 카드의 썸네일은 공식 스크린샷 주소를 사용한다. 새로 만든 콘텐츠의 썸네일은 해당 코드의 실행 결과에서 별도 생성한다.
