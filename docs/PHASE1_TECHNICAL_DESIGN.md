...
Phase 1의 공개 콘텐츠는 원본 CodePen Embed로 실행한다. 목록에서는 CodePen 공식 스크린샷을 예제 선택용 썸네일로 보여주고 클릭 시 같은 자리에서 실행 iframe으로 전환한다. 썸네일, 녹화 영상 또는 GIF를 실행 화면 대신 사용하지 않는다. 자체 코드 실행 환경은 격리 출처와 보안 정책이 필요한 Phase 2에서 구축한다.
