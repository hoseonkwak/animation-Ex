...
- 목록은 CodePen 공식 스크린샷을 지연 로딩하고 iframe을 자동으로 만들지 않는다.
- 썸네일을 누르면 그 카드에서 실제 CodePen 실행 iframe을 표시한다. 썸네일 로딩 실패에도 실행 버튼을 유지한다.
- 동시에 실제 실행하는 Preview는 기본 2개로 제한한다.
- 사용자가 누른 카드를 우선하고 세 번째 실행 시 가장 먼저 실행한 카드로 썸네일을 되돌린다.
...
- 실행이 실패해도 썸네일을 실행 완료 화면인 것처럼 표시하지 않는다. 다시 실행과 원본 링크를 제공한다.
